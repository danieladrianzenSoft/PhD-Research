...
%     V_01 = V_0;
    ...
    %tic
    [t,V] = ode15s(@(t,V) dvdt(t,V,IC,realx,h_G1,h_E,h_S,indG,indE,indS,numx,W,L,... %Geometry
            D_G,D_E,D_S,k_D,k_b,phiGE,phiES,... %Viral Transport
            lambda,dt,rho,c,del,w,eta,lambdal,dl,a,T_0,... %Viral Dynamics
            Dd_G,Dd_E,Dd_S,phid_GE,phid_ES,kd_D,kd_B,kd_L,Vb,... %TFV Transport
            Kon, Koff,r, phiDP_E,phiDP_S,tau_0), t, IC,opts1); %TFV-DP production
...
dxG = realx(2)-realx(1);
dxE = realx(indG+3)-realx(indG+2);
dxS = realx(indG+indE+3)-realx(indG+indE+2);
...
dVdt = zeros(length(t),length(IC));
dVdx = zeros(1,length(IC));
d2Vdx2 = zeros(1,length(IC));

%cellInfections = [1,2,3,4,5];

%% VIRUS

for ii = 1 %y direction BC zero flux (dcdx=0)
    dVdt(:,ii) = 2*D_G.*(V(ii+1,:)-V(ii,:))./(dxG.^2);
    %dVdt(i) = 2*D_G.*(V(i+1)-V(i))./(dx.^2);
end
for ii = 2:indG-1 %in gel
    dVdx(ii) = (V(ii+1,:)-V(ii,:))./dxG;
    d2Vdx2(ii) = (V(ii+1,:)-2.*V(ii,:)+V(ii-1,:))./(dxG.^2);
    dVdt(:,ii) = D_G.*d2Vdx2(ii)-(k_D.*V(ii,:));
end
for ii = indG %right before interface - gel
    dVdx(ii) = (V_intf1a - V(ii,:))./dxG;
    d2Vdx2(ii) = (V(ii-1,:)-2.*V(ii,:)+V_intf1a)./(dxG.^2);
    dVdt(:,ii) = D_G.*d2Vdx2(ii)-(k_D.*V(ii,:));
end
for ii = (indG+1) %right after interface -in epithelium
    dVdx(ii) = (V(ii+1,:)-V(ii,:))./dxE;
    d2Vdx2(ii) = ((V_intf1b)-2.*V(ii,:)+V(ii+1,:))./(dxE.^2);
    dVdt(:,ii) = D_E.*d2Vdx2(ii);

end
for ii = (indG+2):(indG+indE)-1 %in epithelium
    dVdx(ii) = (V(ii+1,:)-V(ii,:))./dxE;
    d2Vdx2(ii) = (V(ii+1,:)-2.*V(ii,:)+V(ii-1,:))./(dxE.^2);
    dVdt(:,ii) = D_E.*d2Vdx2(ii);
end  
for ii = indG+indE %right before interface - epithelium,stroma
    dVdx(ii) = (V_intf2a-V(ii,:))./dxE;
    d2Vdx2(ii) = (V(ii-1,:)-2.*V(ii,:)+V_intf2a)./(dxE.^2);
    dVdt(:,ii) = D_E.*d2Vdx2(ii);
end  
for ii = (indG+indE+1) %right after interface -in stroma
    dVdx(ii) = (V(ii+1,:)-V(ii,:))./dxS;
    d2Vdx2(ii) = ((V_intf2b)-2.*V(ii,:)+V(ii+1,:))./(dxS.^2);
    dVdt(:,ii) = D_S.*d2Vdx2(ii) - k_b.*V(ii,:) + rho*V(ii+(indS+indG+indE+indS+indE+indS+indS+indS),:);
    %dVdt(i) = D_S.*d2Vdx2(i) - k_b.*V(i);

end
for ii = (indG+indE+2):numx-1 %in stroma
    dVdx(ii) = (V(ii+1,:)-V(ii,:))./dxS;
...
for ii = numx%end of tissue
    ...
    dVdt(:,ii) = 2*D_S*(V(ii-1,:)-V(ii,:))./(dxS.^2); %BC zero flux
    %dVdt(i) = 2*D_S*(V(i-1)-V(i))./(dx.^2) - k_b.*V(i); %BC zero flux
end
%% TFV
for ii = numx+1 %TFV at x = 0, zero flux B.C.
    %dVdt(i) = 2*Dd_G.*(V(i+1,:)-V(i,:))./(dxG.^2)-(kd_D.*V(i,:));
    %dVdt(i) = 2*Dd_G.*(V(i+1,:)-V(i,:))./(dxG.^2)-(kd_D.*V(i,:));
    dVdt(ii) = 2*Dd_G.*(V(ii+1,:)-V(ii,:))./(dxG.^2);
end
for ii = numx+2:numx+indG-1 %TFV in gel
    dVdx(ii) = (V(ii+1,:)-V(ii,:))./dxG;
    d2Vdx2(ii) = (V(ii+1,:)-2.*V(ii,:)+V(ii-1,:))./(dxG.^2);
    dVdt(:,ii) = Dd_G.*d2Vdx2(ii)-(kd_D.*V(ii,:));
end
for ii = numx+indG %right before interface - gel/epithelium
    dVdx(ii) = (C_intf_GEa - V(ii,:))./dxG;
    d2Vdx2(ii) = (V(ii-1,:)-2.*V(ii,:)+C_intf_GEa)./(dxG.^2);
    dVdt(:,ii) = Dd_G.*d2Vdx2(ii)-(kd_D.*V(ii,:));
end

for ii = (numx+indG+1) %right after interface - gel/epithelium
    dVdx(ii) = (V(ii+1,:)-V(ii,:))./dxE;
    d2Vdx2(ii) = ((C_intf_GEb)-2.*V(ii,:)+V(ii+1,:))./(dxE.^2);
    dVdt(:,ii) = Dd_E.*d2Vdx2(ii);
end
for ii = (numx+indG+2):(numx+indG+indE-1) %in epithelium
    dVdx(ii) = (V(ii+1,:)-V(ii,:))./dxE;
    d2Vdx2(ii) = (V(ii+1,:)-2.*V(ii,:)+V(ii-1,:))./(dxE.^2);
    dVdt(:,ii) = Dd_E.*d2Vdx2(ii);
end  
for ii = numx+indG+indE %right before interface - epithelium/stroma
    dVdx(ii) = (C_intf_ESa - V(ii,:))./dxS;
    d2Vdx2(ii) = (V(ii-1,:)-2.*V(ii,:)+C_intf_ESa)./(dxS.^2);
    dVdt(:,ii) = Dd_E.*d2Vdx2(ii);
end
for ii = numx+indG+indE+1 %right after interface - epithelium/stroma
    dVdx(ii) = (V(ii+1,:)-V(ii,:))./dxS;
    d2Vdx2(ii) = ((C_intf_ESb)-2.*V(ii,:)+V(ii+1,:))./(dxS.^2);
    dVdt(:,ii) = Dd_S.*d2Vdx2(ii)- kd_B.*V(ii,:);
end
for ii = numx+indG+indE+2:numx+indG+indE+indS-1 %in stroma
    dVdx(ii) = (V(ii+1,:)-V(ii,:))./dxS;
    d2Vdx2(ii) = (V(ii+1,:)-2.*V(ii,:)+V(ii-1,:))./(dxS.^2);  
    dVdt(:,ii) = Dd_S.*d2Vdx2(ii)- kd_B.*V(ii,:);
end  
for ii = (numx+indG+indE+indS)%end of stroma
    ...
    dVdt(:,ii) = 2*Dd_S*(V(ii-1,:)-V(ii,:))./(dxS.^2); %BC zero flux
end

%% TFV-DP

for ii = (numx+indG+indE+indS)+1:(numx+indG+indE+indS)+indE %in epithelium
    bracks = (V(ii-indE-indS,:)*phiDP_E-V(ii,:)/r);
    if bracks < 0
        bracks = 0;
    end
    dVdt(:,ii) = Kon*bracks-Koff*V(ii,:);
    
end
for ii = (numx+indG+indE+indS)+indE+1:(numx+indG+indE+indS)+indE+indS %in stroma
...
tar = V((numx+indG+indE+indS)+indE+indS+1:(numx+indG+indE+indS)+indE+2*indS,:);
%inf = V((numx+indG+indE+indS)+indE+3*indS+1:(numx+indG+indE+indS)+indE+4*indS,:);
MultConcCollisions = (tar.*Vs)';
%MultConcCollisions = zeros(1,length(tar))';
...
for ii = (numx+indG+indE+indS)+indE+indS+1:(numx+indG+indE+indS)+indE+2*indS %target cells
    
    %dVdt(:,i) = lambda-dt*V(i,:)-beta*V(i,:).*V(i-indS,:);
    %dVdt(:,i) = lambda-dt*V(i,:)-cellInfections-collisionsCC_t;
    jj = ii-((numx+indG+indE+indS)+indE+indS);
    dVdt(:,ii) = lambda-dt*V(ii,:)-cellInfections(jj);

end
for ii = (numx+indG+indE+indS)+indE+2*indS+1:(numx+indG+indE+indS)+indE+3*indS %latent cells
    
    %dVdt(:,i) = eta*beta*V(i-indS,:).*V(i-indS-indS,:)-dl*V(i,:)-a*V(i,:);
    jj = ii-((numx+indG+indE+indS)+indE+2*indS);
    dVdt(:,ii) = eta*cellInfections(jj)-dl*V(ii,:)-a*V(ii,:);

end
for ii = (numx+indG+indE+indS)+indE+3*indS+1:(numx+indG+indE+indS)+indE+4*indS %infected cells
    ...
    jj = ii-((numx+indG+indE+indS)+indE+3*indS);
    dVdt(:, ii) = (1-eta)*cellInfections(jj)-del*V(ii,:)+a*V(ii-indS,:);
